...
int main(int argc, char *argv[]) {
  ...
  try {
    ...
    for (; !spk2utt_reader.Done(); spk2utt_reader.Next()) {
      ...
      //ivector 对同一个spker提取自适应状态, 在同一个spker不同utt中可以快速提取ivector
      OnlineIvectorExtractorAdaptationState adaptation_state(ivector_info);

      // foreach utt in spker
      for (size_t i = 0; i < uttlist.size(); i++) {
        ...
        // 设置自适应状态(同一个spker 可以保存之前信息快速提取特征)
        ...
        // 输出utt 的ivector特征 [num_ivectors X 100]
        Matrix<BaseFloat> ivectors(num_ivectors,
                                   ivector_feature.Dim());
        
        // 生成utt的 ivector ==> ivectors [num_ivectors X 100]
        for (int32 i = 0; i < num_ivectors; i++) {
          // 提取ivector 特征的起始帧index
          int32 t = i * n;
