...
// 提取一个ivector 不仅需要 IvectorExtractor, 还需要从GMM中计算后验概率
// GMM(UBM)必须是一个fullGMM.
// 后验概率的计算是, 先利用DiagGmm进行高斯分量选择, 然后在通过FullGMM获得后验概率.
...
class IvectorExtractor {
 ...
 protected:
  ...
  /// Ivector-subspace ivector子空间映射矩阵(定义了全异变空间)
  ...
  /// 说话人自适应模型的 协方差矩阵的逆矩阵 I 高斯分量, D MFCC特征为度
  ...
  // 每个高斯分量 对数似然求解公式中的 常数部分.
  /// The constant term in the log-likelihood of each Gaussian (not
  /// counting any weight).
  Vector<double> gconsts_;

  /// U_i 这个量的计算公式是 M_i^T * Sigma_i^{-1} * M_i
  /// U_i 是在ivector估计中使用的量
  /// 原理上U_i 应该是 vector<SpMatrix> 和 M 相同维度的数表, 但是通过SpMatrix矩阵保存其中的对角元素
  /// 用Matrix中的行即可表示 vector<SpMatrix>
  ///
  /// ??? 下面写的公式不太对, 应该是上面写的
...
class IvectorExtractorStats {
 ...
 protected:
  ...
  /// 每个gaussion分量的 二阶统计量, 我们会用来更新 Sigma_inv_
