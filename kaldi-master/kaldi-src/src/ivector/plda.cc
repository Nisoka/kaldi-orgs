...
void PldaStats::AddSamples(double weight,
  ...
  // weight = 1.0
  // group spk 的所有utt ivector Mat
  // spk1
  //     [utt1 ivector-dim 100]
  //     [utt2 ivector-dim 100]
  //     [utt3 ivector-dim 100]


  if (dim_ == 0) {
      // dim_ == ivector.dim=100
    Init(group.NumCols());
  } else {
  ...
  // group^T * group 计算各维特征 累乘矩阵(协方差的一部分)
  ...
  // spk类别信息 (spker 的 各维度均值, utt数量)
  // mean 就代表一个 spker?
  class_info_.push_back(ClassInfo(weight, mean, n));

  num_classes_ ++;
  num_examples_ += n;
  // class_weight = spker_cnt
  class_weight_ += weight;
  // example_weight = utt_cnt
