...
IvectorExtractor::IvectorExtractor(
  ...
  // UBM 的 means 每个分量 一个mean向量(feats-dim)
  ...
  int32
      feature_dim = Sigma_inv_[0].NumRows(),
  ...
  for (int32 i = 0; i < num_gauss; i++) {
    // ivector-dim => feat-dim的变换 所以维度是 [dim x ivector_dim]
    M_[i].Resize(feature_dim, opts.ivector_dim);
    // 随机高斯分布初始化矩阵
    M_[i].SetRandn();
    // 每个高斯分量 的矩阵为 [dim X ivector_dim], 将高斯分量mean均值向量拷贝到M_[i]的第0列,
    // gmm_means.Row(i) 保存的第i个高斯分量的矩阵向量维度:[feat-dim]
...
void IvectorExtractor::ComputeDerivedVars() {
  ...
  // 以前也尝试过使用RunMultiThreaded方式实现下面的功能, 但是发现有一些问题 不好同步.
  ...
  // could because some tasks finish before others.
  {
    ...
    // 计算推导变量的类型对象
    TaskSequencer<IvectorExtractorComputeDerivedVarsClass>
        sequencer(sequencer_opts);
    // 每个高斯分量独立线程 计算, 实际最终会调用下面的 IvectorExtractor::ComputeDerivedVars(int32 i)
    for (int32 i = 0; i < NumGauss(); i++)
...
/**
 * @brief OnlineIvectorEstimationStats::AccStats
 * @param extractor
 * @param feature       lda 变换后特征
 * @param gauss_post    最佳的高斯后验概率
 */
void OnlineIvectorEstimationStats::AccStats(
  ...
  int32
      ivector_dim = this->IvectorDim(),
      quadratic_term_dim = (ivector_dim * (ivector_dim + 1)) / 2;

  // quadratic_term_ 就是全异变空间矩阵?
...
void OnlineIvectorEstimationStats::GetIvector(
  ...
  if (num_frames_ > 0.0) {
    ...
    // 执行一个梯度下降 计算 ivector 计算过程是
    //                A               b               x
    //  A x = b, 最终目标是 A x - b <= max_error.
    //  求解出 x 即可
    //  这样就就提取了 ivector 特征
    //  那么说明 linear_term_ 是自适应好的 spker超矢量 - ubm均值超矢量
    //  M(spker) = m + V*ivec + e(忽略)
    //  ==> linear_term = M(spker) - m
    //  ivector 就是全异变空间因子
...
void IvectorExtractorUtteranceStats::AccStats(
  ...
  for (int32 t = 0; t < num_frames; t++) {
    ...
    //迭代最高的几个高斯分量的后验概率(是对数似然值 经过softmax之后的后验概率值)
    for (VecType::const_iterator iter = this_post.begin();
      ...
      // Fc'_i是个vector = P(?) * vector<Xt>
      // 经过sum 可以得到 Fc'_i = sum{P(?) * Xt} 一阶统计量
