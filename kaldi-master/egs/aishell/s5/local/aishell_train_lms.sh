...
############################################################
# 创建text.no_oov
# 所有text 中的词 判断是否存在与 lexicon中, 存在的写入 text.no_oov
# 不存在的 写 <SPOKEN_NOISE> 进入text.no_oov
# text.no_oov 中和 text标注差不多, 不过其中未在lexicon出现的词被替换为 <SPOKEN_NOISE>
# 统计词出现总数, 用于训练语言模型概率
############################################################
...
# 比word.counts 增加在lexicon.txt 中的出现次数
