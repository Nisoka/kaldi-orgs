...
if [[ ! -f $srcdir/lexicon.txt ]]; then
  ...
  # 如下正则表达式 (\S+\s+) 表示匹配的第一个$1 (.+) 表示匹配的第二个 $2 中间的 \S+\s+ 匹配模式没有输出$n结果
  # 所以取的实际上是第一个 任意非空字符+任意空字符 的组合, 以及最后的任意字符部分, 去掉中间被 ' '分割的 1.0
...
##########################################################
#　创建 phone_map.txt
#　将src phone 映射为 dest phone , 包含sil nonsil 即所有phone 一般没什么必要
##########################################################
# false
if $position_dependent_phones; then
  ...
  #  将所有 phone 都写入phones文件, 然后做一个直接映射到自己的文件 phone_map.txt
  #  a  a
  #  aa1 aa1
...
# false, 不允许silence 共享GMM, 是为了多个sil情况的, 一般都false
if $share_silence_phones; then
  ...
  # 设置 silsence phone 每个具有不同的 GMM模型参数, 设置标记 shared split sil
...
# 应该不存在 extra_questions.txt, 至此, 不会生成 data/lang/phones/extra_questions.txt
# if extra_questions.txt is empty, it's OK.
# 应该后面会继续操作extra_question.txt, 然后会写入一些集合
...
#生成 $dir/phones/disambig.txt  保存所有消岐符号
...
##########################################################
#　创建　 data/lang/phones.txt ,
#　为每个phone 创建　id, 加上disambig 符号id
##########################################################
...
if [[ ! -z $phone_symbol_table ]]; then
  ...
  # 把所有 silence  nonsilence disambig 的符号 都标号id > phones.txt
...
##########################################################
#　创建　 word-boundary.txt 文件
#　有关词　经常出现在语句中的位置的信息，　一般不提供
##########################################################

# Create a file that describes the word-boundary information for
# each phone.  5 categories.
# false  创建一个 描述每个 phone 的 word-boundary信息的文件, 一般不需要
if $position_dependent_phones; then
...
# 从 lexiconp.txt 中去除每个词, 然后顺序为每个词生成 id
...
##########################################################
#　创建  align_lexicon.txt
#  利用words.txt phones.txt 等生成 id形式的 align_lexicon.int, 实际就是查表映射
##########################################################

# 如果没由使用 word-postion-dependent 的phones,
# 那么可以使用 align_lexicon 来操作 lattice word alignment
...
##########################################################
#　映射 oov.txt  wdisambig.txt silence nonsilence
#    optional_silence disambig context_indep roots word_boundary
# 等都生成对应的 id形式
##########################################################
# 未知词  <SPOKEN_NOISE>
...
# 创建 带消岐符号的lexicon.fst, 需要一个额外的步骤
# 我们创建一个通过消岐符号的
