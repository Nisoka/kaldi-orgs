...
int main(int argc, char *argv[]) {
  try {
    ...
    // 实际就是copy eg 可能会发生 一个eg 被分散到 多个eg
    // 或者实际应用中 可以将eg 读取出来 作为别的bin的输入.
    const char *usage =
    ...
    // 如果设置如下变量, 可以用来从一个 多帧样本中 抽取带labels的单帧
    // 也可以减少一些上下文信息
    ...
    // 可以通过给帧设置一个num,来选择一个确定offset偏移的frame, 或者随机选择一个随机帧frame
    ...
    std::string
        frame_str,
        eg_weight_rspecifier,
        eg_output_rspecifier;
    ...
    // foreach  example.( frames_per_eg = 8)
    for (; !example_reader.Done(); example_reader.Next(), num_read++) {
      ...
      // 正常情况下, 只使用eg_orig, eg_modified_output 只是用在多语言同时训练时.
      ...
      for (int32 c = 0; c < count; c++) {
        ...
        if (frame_str == "" && left_context == -1 && right_context == -1 &&
          ...
          // index 是对 num_outputs(多个输出) 求余, 表示循环向多个文件输出.
