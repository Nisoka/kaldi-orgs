...
int main(int argc, char *argv[]) {
  try {
    ...
    for (; !feature_reader.Done(); feature_reader.Next()) {
      ...
      //每帧 保存对应的最大的num_post个高斯分量对数似然值
      //
      vector<vector<int32> > gselect(T);
      
      Matrix<BaseFloat> loglikes;
      
      // loglikes = [ frame_cnt X num_guass ] 保存每帧数据的 每个高斯分量的对数似然值
      // 计算每帧的 每个高斯分量的对数似然值
      ...
      //每帧计算  num_post 目标保留5个最佳概率值的高斯分量,
      // 经过softmax 变为每个高斯分量的后验概率, min_post 最小的后验概率
      for (int32 t = 0; t < T; t++) {
        log_like_this_file +=
