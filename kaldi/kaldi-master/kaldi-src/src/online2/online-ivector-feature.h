...
struct OnlineIvectorExtractionConfig {
  ...
  // 如果true, 返回最近可用的ivector(表示真的处理完成的), 而不是当前帧的?
  // 意味着如果如果 音频速度超过处理速度, 我们会返回一个更准确的ivector??(虽然可能不是当前帧的)
...
struct OnlineIvectorExtractionInfo {
  ...
  BaseFloat max_count;
  int32 num_cg_iters;    //15 固定的,不重要
  bool use_most_recent_ivector;  //true
...
/// 保存 online ivector extractor的自适应状态, 帮助初始化相同spker 的下一个utt的自适应状态
...
struct OnlineIvectorExtractorAdaptationState {
  ...
  /// 此类 是为了对相同spker的下一个utt快速 进行自适应得到 ivector?
...
class OnlineIvectorFeature: public OnlineFeatureInterface {
 ...
 private:
  ...
  // 我们从当前utt已经计算过 并加入进 ivector_stats_统计量中的 帧总数
  // t < num_frames_stats_ 的帧都已经进入 ivector_stats_中了.
