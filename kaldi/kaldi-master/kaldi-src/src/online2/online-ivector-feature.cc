...
void OnlineIvectorExtractionInfo::Init(
  ...
  use_most_recent_ivector = config.use_most_recent_ivector;  //true
...
void OnlineIvectorFeature::UpdateStatsForFrame(int32 t,
  ...
  Vector<BaseFloat>
      feat(feat_dim),  // features given to iVector extractor
      log_likes(info_.diag_ubm.NumGauss());

  //lda_normalize 处理第t帧特征, 对拼接数据进行线性变换lda正则化 然后输出到 feat
  lda_normalized_->GetFrame(t, &feat);

  // ubm 计算feat 的 各个高斯分量的对数似然
  info_.diag_ubm.LogLikelihoods(feat, &log_likes);

  // 计算后验概率, 剪枝最重要的 num_gselect 个高斯后验概率
  ...
  //特征变换LDA 与上面的 lda_normalized 不同在于
  //这里lda_的是原始特征  raw + splice,
  // lda_normalized 是 经过CMVN均值归一化的  cmvn + splice, 因为 ubm 需要的是 归一化的
...
/**
 * @brief OnlineIvectorFeature::GetFrame
 * @param frame  提取ivector特征的起始帧 index
 * @param feat  提取出的ivector
 */
