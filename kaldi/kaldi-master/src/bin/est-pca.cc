  ...
  try {
    ...
    if (!read_vectors) {
      ...
      for (; !feat_reader.Done(); feat_reader.Next()) {
        ...
        // 将 mat的所有frame的对应特征累和 + sum
        // sum保存 mat的所有特征, 并将所有帧对应特征累和.
    ...
    //计算pca 主成分分析, 实现降维, 按主成分重要度(特征值)排序
    sumsq.Eig(&s, &P);
    SortSvd(&s, &P);
    // 特征值 s, 特征向量矩阵 P
    ...
    //归一化均值
    if (normalize_mean) {
      //this <– 0*this + -1.0*transform[91 x 91]*v[91x1].
    ...
    //transform_float 是已经按照特征值大小排序的, 所以只保留前 dim 个特征向量即可.
