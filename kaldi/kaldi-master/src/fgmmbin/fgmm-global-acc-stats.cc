...
int main(int argc, char *argv[]) {
  try {
    ...
    for (; !feature_reader.Done(); feature_reader.Next()) {
      ...
      BaseFloat
          file_like = 0.0,
          file_weight = 0.0; // total of weights of frames (will each be 1 unless --weights option supplied).
      ...
      if (gselect_rspecifier != "") {
        ...
        for (int32 i = 0; i < file_frames; i++) {
          ...
          // 内部只计算了 30个分量的概率, 其他的是因为太小了 直接不更新了, 因为更新也没什么作用.
          // 计算 预先选择的30个高斯分量概率 向量 => loglikes
          fgmm.LogLikelihoodsPreselect(data, this_gselect, &loglikes);

          // 对高斯分量概率 向量 应用 softmax 直接获得对应的响应度
          file_like += weight * loglikes.ApplySoftMax();
          loglikes.Scale(weight);

          // 更新 更新式统计量, 等待直接进行更新, 貌似
