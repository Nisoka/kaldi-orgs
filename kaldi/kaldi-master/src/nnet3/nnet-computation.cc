...
int32 ComputationRequest::IndexForInput(const std::string &node_name) const {
	int32 ans = -1;
	for (size_t i = 0; i < inputs.size(); i++) {
...
int32 ComputationRequest::IndexForOutput(const std::string &node_name) const {
	int32 ans = -1;
...
void NnetComputation::ComputeCudaIndexes() {
	...
	for (int32 i = 0; i < indexes_ranges.size(); i++) {
		const std::vector<std::pair<int32, int32> > *input =
				&(indexes_ranges[i]);
...
int32 NnetComputation::NewSubMatrix(int32 base_submatrix, int32 row_offset,
		int32 num_rows, int32 col_offset, int32 num_cols) {
	KALDI_ASSERT(
			base_submatrix > 0
					&& static_cast<size_t>(base_submatrix)
							< submatrices.size());
	...
	KALDI_ASSERT(
			row_offset + num_rows <= base_info.num_rows
	...
	submatrices.push_back(
			NnetComputation::SubMatrixInfo(base_matrix, matrix_row_offset,
					num_rows, matrix_col_offset, num_cols));
	return ans;
}
...
int32 NnetComputation::NewMatrix(int32 num_rows, int32 num_cols,
	...
	int32 matrix_index = matrices.size(), submatrix_index = submatrices.size();
...
void NnetComputation::MatrixDebugInfo::Write(std::ostream &os,
		bool binary) const {
...
void NnetComputation::SubMatrixInfo::Write(std::ostream &os,
		bool binary) const {
...
void NnetComputation::GetSubmatrixStrings(const Nnet &nnet,
	...
	for (int32 i = 1; i < num_submatrices; i++) {
		...
		if (this->IsWholeMatrix(i)) {
			...
			os << 'm' << submat.matrix_index << '(' << submat.row_offset << ':'
					...
					<< submat.col_offset << ':'
					<< (submat.col_offset + submat.num_cols - 1) << ')';
		}
...
static void GetIndexesMultiStrings(const Nnet &nnet,
		const NnetComputation &computation,
	...
	for (int32 i = 0; i < indexes_multi_size; i++) {
		...
		for (int32 j = 0; j < size; j++) {
			...
			if (submat_index == -1) {
				...
				int32 col_start = submat.col_offset, col_end = col_start
						+ submat.num_cols;
				if (!(row_index < submat.num_rows && row < mat.num_rows)) {
...
static void PrintCommand(std::ostream &os, const Nnet &nnet,
		const NnetComputation &computation, int32 command_index,
	...
	case kDeallocMatrix:
		...
		break;
	case kSwapMatrix:
		os << submatrix_strings[c.arg1] << ".swap(" << submatrix_strings[c.arg2]
				<< ") [dim = " << computation.submatrices[c.arg1].num_rows
				<< " x " << computation.submatrices[c.arg1].num_cols << "]\n";
		break;
	case kSetConst:
	...
	case kMatrixCopy:
		if (c.alpha == 1.0) {
			...
			os << submatrix_strings[c.arg1] << " = " << c.alpha << " * "
					<< submatrix_strings[c.arg2] << "\n";
		}
		break;
	case kMatrixAdd:
	...
	case kCopyRows:
		os << submatrix_strings[c.arg1] << "."
				<< (c.command_type == kAddRows ? "AddRows" : "CopyRows") << "("
				<< c.alpha << ", " << submatrix_strings[c.arg2]
		...
		os << submatrix_strings[c.arg1] << "."
				<< (ct == kAddRowsMulti ?
						...
						(ct == kAddToRowsMulti ?
								"AddToRowsMulti" :
								(ct == kCopyRowsMulti ?
										"CopyRowsMulti" : "CopyToRowsMulti")))
				<< "(" << c.alpha << ", " << indexes_multi_strings[c.arg2]
				<< ")\n";
		break;
	}
	case kAddRowRanges: {
		os << submatrix_strings[c.arg1] << ".AddRowRanges(" << c.alpha << ", "
				<< submatrix_strings[c.arg2] << ", [";
...
static void PrintComputationPreamble(std::ostream &os, const NnetComputation &c,
		const Nnet &nnet, const std::vector<std::string> &submatrix_strings,
		const std::vector<std::string> &indexes_strings,
		const std::vector<std::string> &indexes_multi_strings) {
...
void NnetComputation::Print(std::ostream &os, const Nnet &nnet) const {
	...
	for (int32 c = 0; c < commands.size(); c++) {
		PrintCommand(os, nnet, *this, c, submatrix_strings, indexes_strings,
				indexes_multi_strings);
	}
}
...
void NnetComputation::Read(std::istream &is, bool binary) {
	...
	if (tok == "<ComponentPrecomputedIndexes>") {
		...
		for (size_t c = 0; c < num_component_precomputed_indexes; c++) {
			...
			if (!is_null) {
				ComponentPrecomputedIndexes* p =
						ComponentPrecomputedIndexes::ReadNew(is, binary);
...
void NnetComputation::GetCommandStrings(const Nnet &nnet, std::string *preamble,
...
bool NnetComputation::SubMatrixInfo::operator==(
	...
	return matrix_index == other.matrix_index && row_offset == other.row_offset
			&& num_rows == other.num_rows && col_offset == other.col_offset
...
IoSpecification::IoSpecification(const std::string &name, int32 t_start,
		int32 t_end) :
		name(name), indexes(std::max<int32>(0, t_end - t_start)), has_deriv(
				false) {
...
NnetComputation::NnetComputation(const NnetComputation &other) :
		matrices(other.matrices), matrix_debug_info(other.matrix_debug_info), submatrices(
				other.submatrices), component_precomputed_indexes(
				other.component_precomputed_indexes), indexes(other.indexes), indexes_multi(
				other.indexes_multi), indexes_ranges(other.indexes_ranges), commands(
				other.commands), need_model_derivative(
				other.need_model_derivative), indexes_cuda(other.indexes_cuda), indexes_ranges_cuda(
				other.indexes_ranges_cuda) {
	for (size_t i = 1; i < component_precomputed_indexes.size(); i++)
...
void NnetComputation::GetWholeSubmatrices(
	...
	int32 num_matrices = matrices.size(), num_submatrices = submatrices.size();
	...
	for (int32 m = 1; m < num_matrices; m++) {
		KALDI_ASSERT(
				(*whole_submatrices)[m] != 0
						&& "Matrix exists with no submatrix that is "
								"the whole of it.");
	}
}

size_t IoSpecificationHasher::operator ()(const IoSpecification &io_spec) const
		noexcept {
	StringHasher string_hasher;
	IndexVectorHasher indexes_hasher;
	// 4261 was chosen at random from a list of primes.
	return string_hasher(io_spec.name) + indexes_hasher(io_spec.indexes)
