...
function do_filtering {
  ...
  # 一般存在 srcdir/wav.scp , 将destdir 需要的utt(通过utt2spk文件过滤) 过滤srcdir/wav.scp提取写入到 destdir/wav.scp
  # 这样destdir/wav.scp 中只保存 utt2spk 中存在的 wav文件位置索引.
...
if $spk_list_specified; then
  ...
  # 在提供 perspk option时, 选择 每个spker的 numutt个utt wav文件 eg:5
  # 生成spk2utt
  # 将 spker 的5 个wav文件 写入 spk2utt
  # spk1  utt1 utt2 utt3 utt4 utt5
  # 生成utt2spk
  # spk2utt_to_utt2spk.pl 生成反向索引 utt2spk
  mkdir -p $destdir
